...
class EntityApi(object):
    ...
    def get_entity_expand(self, **kwargs):
        """Get an expand from prov-dm entity
        ...
        for (key, val) in params['kwargs'].iteritems():
            if key not in allParams:
                raise TypeError("Got an unexpected keyword argument '%s' to method get_entity_expand" % key)
            params[key] = val
        del params['kwargs']

        resourcePath = '/entity/expand'
        resourcePath = resourcePath.replace('{format}', 'json')
        method = 'GET'
